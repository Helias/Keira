...
if ($entry != "")
{
	...
	while ($row=mysql_fetch_array($query))
	{
		if($row['RandomProperty'] != 0 OR $row['RandomSuffix'] != 0)
		{
			...
			{
				$query2=mysql_query("SELECT * FROM item_enchantment_template WHERE entry={$row['RandomProperty']}");
			}
			else
			{
				$query2=mysql_query("SELECT * FROM item_enchantment_template WHERE entry={$row['RandomSuffix']}");
			}
...
function Scripts()
{
	...
	if(entries != "")
	{
		if(insert != "")
		{
			Script="DELETE FROM `item_enchantment_template` WHERE entry="+id+" AND item IN ("+entries+"); <br>INSERT INTO `item_enchantment_template` (`entry`,`ench`,`chance`) VALUES"+insert;
			Script=Script.substr(0, Script.length-1);
			Script+=";";
		}
		else
		{
			Script="DELETE FROM `item_enchantment_template` WHERE entry="+id+" AND item IN ("+entries+");";
		}
	}
